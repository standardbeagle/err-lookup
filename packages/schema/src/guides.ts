 ...
 * pages, the sitemap, and the info-page collector's link-outs — so adding a
 * guide is one entry plus one article. It lives in schema so the site and the
 * pipeline cannot drift on which guides exist.
 */
...
/** Guides linked from one error page or info page. */
